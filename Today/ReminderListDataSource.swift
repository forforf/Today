...
class ReminderListDataSource: NSObject {
    ...
    func storeChanged(_ notification: NSNotification) {
        // Is it possible to DRY requestAccess as it's used multiple times?
        requestAccess { authorized in
            if authorized {
                self.readAllReminders()
            }
    ...
    //It might be useful to map how the completion closures are being passed around
...
extension ReminderListDataSource: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.configure(title: currentReminder.title, dateText: dateText, isDone: currentReminder.isComplete) {
            ...
            // Not sure if rows should be reloaded, it was not explicitly mentioned in the tutorial
            // if this line should be moved into the closure or not, but it was missing when adding
            // the success closure to the update method
...
extension ReminderListDataSource {
    ...
    // As of this writing the isAvailable computed property uses the same EKEventStore.authorizationStatus(for:) method
    ...
    private func saveReminder(_ reminder: Reminder, completion: (String?) -> Void) {
        ...
        readReminder(with: reminder.id) { (ekReminder) in
            ...
            // This app only syncs reminders that have alarms
            // It also does some alarm/due date manipulation that may have some consequences (needs investigation)
